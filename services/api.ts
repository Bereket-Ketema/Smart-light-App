...
const BASE_URL = "http://172.24.42.41:5000";
...
const toLightState = (payload: any): ApiResponse => {
  const state = payload?.data?.state ?? payload?.data;
  return {
    status: state?.power,
    mode: state?.mode,
  };
...
// ========== Send Motion Data ==========
export const sendMotionData = async (
  motionDetected: boolean,
  baseUrl?: string,
): Promise<ApiResponse> => {
  const url = baseUrl || BASE_URL;
  console.log(
    "📤 API Call: sendMotionData - URL:",
    `${url}/bridge/motion-event`,
  );
